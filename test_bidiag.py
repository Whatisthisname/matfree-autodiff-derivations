...
def bidiagonalize_jvp(primals, tangents) -> tuple[BidiagOutput, BidiagOutput]:
    ...
    d_as = [0] * len(as_)
    d_bs = [0] * len(bs)
    d_rs = [any_n_vec.copy() * 0 for _ in range(len(rs))]
    d_rs[1] = (
        d_start_vector - start_vector * (start_vector.T @ d_start_vector)
    ) / np.linalg.norm(start_vector)
    d_ls = [any_m_vec.copy() * 0 for _ in range(len(ls))]
...
def test_bidiag_jvp(seed):
    ...
    result, tangents = bidiagonalize_jvp(
        primals=(A, start_vector), tangents=(d_A, d_start_vector)
    )
...
def test_bidiag_tall_matrix(seed):
    ...
    result, _ = bidiagonalize_jvp((A, start_vector), (A, start_vector))
