...
from exp import configuration_penalty, configuration_penalty_hessian, robot_arm_position
...
for plot_idx, penalty_quantile in enumerate(penalty_quantiles):
    ...
    for i, idx in enumerate(sample_indices):
        x_points, y_points = get_robot_arm(angles[idx])
        # Use different colors for each arm
        color = f"C{i}"
        axs[plot_idx].plot(
            x_points, y_points, linewidth=2, marker="o", markersize=4, color=color
        )
        # Add arrow at the tip
